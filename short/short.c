...
/* detect irq number with help of kernel */
void short_kernelprobe(void)
{
	...
	do {
		...
		if (short_irq == 0) { /* not found and try again (at most 5 times) */
...
/* interrupt handler for self probe */
...
int short_init(void)
{
	...
	if (!use_mem) {
		...
		/* also, ioremap it to make sure I/O memory is accessable to the kernel */
	...
	if ((result = register_chrdev(major, "short", &short_fops)) < 0) {
